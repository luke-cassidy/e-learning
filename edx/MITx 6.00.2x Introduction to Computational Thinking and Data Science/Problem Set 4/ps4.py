...
def generate_models(x, y, degs):
    ...
    coeffs = []
    for deg in degs:
        coeffs.append(np.polyfit(x.copy(), y.copy(), deg))
...
def r_squared(y, estimated):
    ...
    """
    mean = np.mean(y.copy())
    numerator = np.sum(np.square(np.subtract(y.copy(), estimated.copy())))
    denominator = np.sum(np.square(np.subtract(y.copy(), mean)))
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        pylab.plot(x, estY, label = "Degree 1 model" + "\n"
        + "R^2 = " +  str(r_squared(y, estY)))

    pylab.plot(x, y, label = "Sample data")

    pylab.title("Temperature Models")
    pylab.xlabel("Year")
